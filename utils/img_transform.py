# utils/img_transform.py

"""Funcionalidade genérica para suportar transformações de imagens do Cartopy

Foi necessário fazer uma modificação manual nesse módulo do Cartopy para que fosse possível realizar uma transformação da nossa
função GenDataset (a função linspace do numpy presente nas linhas 79 e 81 estava recebendo nx e ny como float, dessa forma,
precisamos adicionar uma transformação para inteiro "int()" antes de passar os valores para a função).
...
def mesh_projection(projection, nx, ny,
    ...
    x, xstep = np.linspace(x_lower, x_upper, int(nx), retstep=True,
                           endpoint=False)
    y, ystep = np.linspace(y_lower, y_upper, int(ny), retstep=True,
                           endpoint=False)
